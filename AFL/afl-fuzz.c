...
EXP_ST void init_forkserver(char** argv) {
  ...
  if (!forksrv_pid) {
    ...
    /* Use a distinctive bitmap signature to tell the parent about execv()
       falling through. */
    // 正常情况下执行不到这里。如果执行到这里，说明 execv 失败了
    *(u32*)trace_bits = EXEC_FAIL_SIG;
    exit(0);

  }
  // 以下是父进程继续执行的代码。父进程会等待 fork server 发来 hello 包，若接收成功，则 fork server 成功启动，初始化完成；否则做一点错误处理。
  ...
  // 设置定时器，等待 fork server 发来 4 字节的 hello 包
  it.it_value.tv_sec = ((exec_tmout * FORK_WAIT_MULT) / 1000);
  it.it_value.tv_usec = ((exec_tmout * FORK_WAIT_MULT) % 1000) * 1000;

  setitimer(ITIMER_REAL, &it, NULL);
  // 注意 read 是阻塞的，会等到 fork server 发来 hello 或者超时为止
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (WIFSIGNALED(status) && !stop_soon) {
    ...
    if (child_timed_out && kill_signal == SIGKILL) {
      append_debug("WIFSIGNALED(status) = %d, child_timed_out && kill_signal == SIGKILL, run_target return FAULT_TMOUT\n", WIFSIGNALED(status));
      return FAULT_TMOUT;
    }
    
    append_debug("WIFSIGNALED(status) = %d, kill_signal = %u, run_target return: FAULT_CRASH\n", WIFSIGNALED(status), kill_signal);
  ...
  if (uses_asan && WEXITSTATUS(status) == MSAN_ERROR) {
    kill_signal = 0;
    append_debug("WEXITSTATUS(status) = %d, run_target return: FAULT_CRASH\n", WEXITSTATUS(status));
    return FAULT_CRASH;
  }

  if ((dumb_mode == 1 || no_forkserver) && tb4 == EXEC_FAIL_SIG) {
    append_debug("run_target return: FAULT_ERROR\n");
    return FAULT_ERROR;
  }
  ...
  append_debug("run_target return: FAULT_NONE\n");
...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
       ...
       "  -s            - segment offset from the start address of the main function\n"
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:s:S:M:x:Q")) > 0)
    ...
    switch (opt) {
      ...
      case 'Q': /* QEMU mode */
        // 首先检查qemu_mode是否已被设置，防止多次设置-Q选项，随后将qemu_mode变量置位
        if (qemu_mode) FATAL("Multiple -Q options not supported");
        qemu_mode = 1;
        // 若mem_limit_given标志位(此标志位通过-m选项设置)未被设置，将mem_limit变量设置为200(MB)
  ...
  // 检查 sync_id 字符串是否合法，并设置 sync_dir 和 out_dir
  ...
  // 若有环境变量 AFL_NO_UI，则 not_on_tty = 1,让它不画 ui，而是逐行输出日志
  check_if_tty();
  // cpu 相关
  get_core_count();

/* fuzzer 检查系统的负载，把自己绑定到一个空闲 cpu 核心上。另外，若 cpu 频率可调，则建议用户将其定在最高频率。
　　如果进程 crash 后会报告给某个程序（例如 /usr/share/apport/apport ），这可能造成延迟，使得 fuzzer 误认为程序超时。AFL 会检查这种情况，并提醒用户。 */
// linux 下 HAVE_AFFINITY=1，绑定到一个空闲 cpu 上
#ifdef HAVE_AFFINITY
  bind_to_free_cpu();
#endif /* HAVE_AFFINITY */
  ...
  // 若发现 cpu 频率可调，则唆使用户把 cpu 定在最高频率
  check_cpu_governor();
  ...
  // dry run，用 queue 中的所有用例跑一遍程序
  perform_dry_run(use_argv);

  // 精简队列，给 queue 中的用例打分，分为 favored 和 non-favored 两类
  cull_queue();
  ...
  // 更新 fuzzer_stats 文件，以人类可读的方式记录了 fuzzer 的当前状态，并随着 fuzz 过程不断更新
