...
class CutNode:
    ...
    @staticmethod
    def fromAngrCutNode(angr_node: angr.knowledge_plugins.cfg.cfg_node.CFGNode):
        node = CutNode()
...
class Function:
    ...
    def fromAngrFunction(angr_function: angr.knowledge_plugins.functions.function.Function, angr_cfg: angr.analyses.cfg.cfg_fast.CFGFast):
        ...
        func.nodes = {addr:CutNode.fromAngrCutNode(angr_cfg.get_any_node(addr)) for addr in func.node_addrs_set}
    ...
    # Modifier
    def appendNode(self, node: CutNode):
        pass

    def removeNode(self, node: CutNode):
        pass
...
class CFG:
    ...
    # Modifier
    def appendCFGNode(self, node: CutNode):
        if node.addr not in self.nodes:
            self.nodes[node.addr] = node
            return True
        return False
    
    def removeCFGNode(self, node: CutNode):
